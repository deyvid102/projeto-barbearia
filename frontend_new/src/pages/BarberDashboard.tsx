...
import { appointments, barbers, weeklyRevenue, serviceDistribution } from '@/data/mockData';
      ...
      <div className="pt-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto pb-16">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Aqui está seu resumo.</p>
          </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="bg-secondary/50 mb-8 flex-wrap h-auto gap-1">
            {[
              { value: 'agenda', label: 'Agenda', icon: BarChart3 },
          ...
          <BarberKPIs commissionToday={commissionToday} agendadosCount={0} pendingCount={pendingToday.length} completedCount={completedToday.length} />
          <ScheduleGrid />
          </TabsContent>
