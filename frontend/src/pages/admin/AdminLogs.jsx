  ...
  return (
    <AdminLayout>
      <div className="p-4 md:p-8 flex flex-col h-full print:block print:bg-white print:p-0">
        ...
        <div className={`mb-6 p-6 rounded-[2rem] border flex flex-wrap gap-4 items-end print:hidden ${isDarkMode ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-200'}`}>
          ...
          {periodo === 'INTERVALO' && (
            <div className="flex gap-2 flex-1 min-w-[240px]">
              <input type="date" value={dataInicio} onChange={(e) => setDataInicio(e.target.value)} className={`text-[10px] font-bold p-3 rounded-xl bg-transparent border h-[46px] outline-none ${isDarkMode ? 'border-white/10 text-white' : 'border-slate-300'}`} />
              <input type="date" value={dataFim} onChange={(e) => setDataFim(e.target.value)} className={`text-[10px] font-bold p-3 rounded-xl bg-transparent border h-[46px] outline-none ${isDarkMode ? 'border-white/10 text-white' : 'border-slate-300'}`} />
            </div>
          )}
        ...
        {/* Tabela */}
        <div className={`flex-1 rounded-[2.5rem] border overflow-hidden print:border-none ${isDarkMode ? 'bg-[#111] border-white/5' : 'bg-white border-slate-200 shadow-2xl'}`}>
          <div className="overflow-x-auto min-h-[400px]">
            <table className="w-full border-collapse">
              <thead>
                <tr className={`${isDarkMode ? 'bg-white/5' : 'bg-slate-50'} print:bg-white print:border-b-2 print:border-black`}>
                  <th className="p-5 text-[10px] font-black uppercase text-gray-500 text-left">Data</th>
                  <th className="p-5 text-[10px] font-black uppercase text-gray-500 text-left">Agendamento</th>
                  <th className="p-5 text-[10px] font-black uppercase text-gray-500 text-left">Cliente</th>
                  <th className="p-5 text-[10px] font-black uppercase text-gray-500 text-center">Status</th>
                  <th className="p-5 text-[10px] font-black uppercase text-gray-500 text-left">Responsável</th>
                </tr>
              </thead>
              <tbody>
                {logsPaginados.map((log) => (
                  <tr key={log._id} className="border-b border-white/5 hover:bg-white/[0.01] transition-colors print:border-slate-200">
                    ...
                    <td className="p-5 text-center">
                      <span className={`text-[9px] font-black px-2 py-1 rounded ${log.status_acao === 'C' ? 'bg-red-500/10 text-red-500' : 'bg-emerald-500/10 text-emerald-500'}`}>
