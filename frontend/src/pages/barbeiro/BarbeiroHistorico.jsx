...
export default function BarbeiroHistorico() {
  ...
  // Filtra apenas Finalizados (F) ou Cancelados (C) e ordena pelos mais recentes
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-gray-100 p-6 font-sans">
      <div className="max-w-md mx-auto space-y-8">
        ...
        <div className="space-y-4">
          {loading ? (
            <p className="text-center text-[10px] text-gray-600 uppercase font-black animate-pulse py-10">
              recuperando registros...
            </p>
          ) : historico.length > 0 ? (
            historico.map(a => (
              <div key={a._id} className="p-6 rounded-[2.5rem] bg-[#111] border border-white/5 flex justify-between items-center transition-all hover:border-white/10">
            ...
            <div className="text-center py-20 border-2 border-dashed border-white/5 rounded-[2.5rem]">
              <p className="text-gray-600 text-[10px] uppercase font-black tracking-widest">nenhum atendimento registrado</p>
            </div>
          )}
        </div>
      </div>
