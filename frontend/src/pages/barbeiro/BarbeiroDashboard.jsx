...
export default function BarbeiroDashboard() {
  ...
  const fetchData = useCallback(async (isAutoRefresh = false) => {
    ...
    try {
      ...
      if (minhaBarbearia) {
        ...
        setConfigLimites({ inicio: parseInt(abertura.split(':')[0]), fim: parseInt(fechamento.split(':')[0]) });
      }
  ...
  // FUNÇÃO CORRIGIDA: Agora recebe o payload pronto do ModalAgendamentoAvulso
  const handleSalvarAvulso = async (payload) => {
    try {
      // O payload já vem com datahora_fim, tipoCorte (camelCase) e IDs corretos do Modal
      ...
      const msgErro = error.response?.data?.error || 'Falha ao processar agendamento.';
      setAlertConfig({ 
        show: true, 
        titulo: 'Erro 400', 
        mensagem: msgErro, 
  ...
  const handleSalvarAcoes = async () => {
    ...
    try {
      const payload = { status: novoStatus, valor: Number(novoPreco) };
  ...
  const getTimelinePositionPercentage = (horaCard) => {
    ...
    if (currentHour === parseInt(cardHour)) {
      ...
      return ((min * 60 + currentTime.getSeconds()) / 3600) * 100;
    }
    return null;
  };

  const getNomeExibicao = (ag) => ag.nomeCliente || clientes.find(c => String(c._id) === String(ag.fk_cliente?._id || ag.fk_cliente))?.nome || 'Cliente';
  const getNomeBarbeiro = (ag) => barbeiros.find(b => String(b._id) === String(ag.fk_barbeiro?._id || ag.fk_barbeiro))?.nome.split(' ')[0] || 'Profissional';
  const agendamentosHoje = agendamentos.filter(a => a.datahora.startsWith(hojeStr));
  const stats = { total: agendamentosHoje.length, agendados: agendamentosHoje.filter(a => a.status === 'A').length, finalizados: agendamentosHoje.filter(a => a.status === 'F').length, cancelados: agendamentosHoje.filter(a => a.status === 'C').length };
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-[#0a0a0a] text-gray-100' : 'bg-gray-50 text-slate-900'} p-4 md:p-8 font-sans transition-colors duration-300`}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative group flex flex-col items-center" ref={menuRef}>
                <button 
                  ...
                  className={`flex items-center gap-3 p-2 px-3 rounded-2xl border transition-all ${isProfileOpen ? 'bg-[#e6b32a] text-black shadow-lg shadow-[#e6b32a]/20' : isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white border-slate-200'}`}
                >
                ...
                {isProfileOpen && (
                  <div className={`absolute right-0 top-full mt-3 w-64 border rounded-[2.2rem] shadow-2xl z-[150] overflow-hidden ${isDarkMode ? 'bg-[#111] border-white/10' : 'bg-white border-slate-100'}`}>
                    <div className="p-4 border-b border-black/5 dark:border-white/5">
                        <p className="text-[9px] font-black uppercase opacity-40 tracking-widest ml-1">Gerenciamento</p>
                    </div>
                    ...
                    <button onClick={() => { localStorage.clear(); navigate('/barbeiro/login'); }} className="w-full p-5 text-[10px] font-black uppercase text-red-500 flex items-center gap-3 bg-red-500/5 hover:bg-red-500/10 transition-colors">
                  ...
                  return (
                    <tr key={hora} className="relative group/row" style={{ height: `${ALTURA_LINHA}px` }}>
                      <td className={`sticky left-0 z-20 p-2 border-b border-r text-center font-mono text-[10px] font-black transition-colors ${isDarkMode ? 'bg-[#111] border-white/5 text-gray-500 group-hover/row:text-[#e6b32a]' : 'bg-slate-50 border-slate-100 text-slate-400 group-hover/row:text-black'}`}>
                        {hora}
                        {posLinha !== null && <div className="absolute left-0 w-[2000px] z-50 pointer-events-none flex items-center" style={{ top: `${posLinha}%` }}><div className="w-full h-[1.5px] bg-red-600 shadow-[0_0_8px_rgba(220,38,38,0.4)]"></div></div>}
                      </td>
                      {barbeiros.map(b => {
                        ...
                        return (
                          <td key={b._id} className={`p-1 border-b border-r align-top relative ${isDarkMode ? 'border-white/5' : 'border-slate-100'}`}>
                            <div className={`grid gap-1 h-full ${ags.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                              {ags.map(ag => (
                                <button key={ag._id} onClick={() => isMeu && openAcoes(ag)} className={`group w-full p-1.5 rounded-lg text-left border shadow-sm transition-all h-fit relative z-10 ${!isMeu ? 'opacity-30 grayscale' : 'hover:scale-[1.02]'} ${ag.status === 'F' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : ag.status === 'C' ? 'bg-red-500/10 border-red-500/20 text-red-500' : 'bg-[#e6b32a] text-black border-[#e6b32a]'}`}>
                                  <div className="flex flex-col">
                                    <div className="flex justify-between items-start">
                                      <p className="text-[8px] font-black uppercase truncate max-w-[70%]">{getNomeExibicao(ag)}</p>
                                      <span className="text-[7px] font-black opacity-70">{new Date(ag.datahora).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</span>
                                    </div>
                                    <div className="mt-1 border-t border-black/5 pt-1 flex justify-between items-end">
                                      <div className="space-y-0.5">
                                        <div className="flex items-center gap-1 opacity-80"><IoCutOutline size={8}/><p className="text-[7px] font-bold uppercase truncate">{ag.tipoCorte || 'serviço'}</p></div>
                                        <div className="flex items-center gap-1"><IoCashOutline size={8}/><p className="text-[8px] font-black">R$ {ag.valor || '0,00'}</p></div>
                                      </div>
                                      <div className="flex items-center gap-1 text-[9px] font-black uppercase text-black/60">{getNomeBarbeiro(ag)}<IoPersonOutline size={9}/></div>
                                    </div>
                                  </div>
                                </button>
                              ))}
      ...
      {/* O segredo está no setAlertConfig resetando o show para false */}
{alertConfig.show && (
  <CustomAlert 
